        ...
        let completed: Driver<WLBaseResult> = input
            ...
            .flatMapLatest {
                ...
                if style == .mix {
                    ...
                    if $0.1.count == 0 { return Driver.just(WLBaseResult.failed("请选择至少一张图片或者视频")) }
                    
                    if !($0.1.contains(where: { $0.type == "image" }) || $0.1.contains(where: { $0.type == "video" }) ) { return Driver.just(WLBaseResult.failed("请选择至少一张图片或者视频")) }
                }
                ...
                return onUserDictResp(WLUserApi.publish(input.tag, content: content))
...
extension WLPublishViewModel {
    ...
    public func replaceContent(_ idx: Int ,pub: WLKeyValueBean) {
