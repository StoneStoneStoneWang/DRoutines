...
// 创建
//- (WLVideoPlayer *)player {
//
//    if (!_player) {
//
//        _player = [WLVideoPlayer videoPlayerWithFrame:CGRectZero andDelegate:self];;
//    }
//    return _player;
//}
...
@protocol WLVideoPlayerDelegate <NSObject>
// 更多按钮事件 代理
- (void)onVideoPlayer:(WLVideoPlayer *)player andMoreBtn:(UIButton *)moreBtn;
// 关闭按钮事件 代理
- (void)onVideoPlayer:(WLVideoPlayer *)player andCloseBtn:(UIButton *)closeBtn;

@end

@interface WLVideoPlayer : UIView
// 创建 video player 对象。 内部做了4g 能否播放的处理 、加载视频失败的处理 视频全屏 都处理好了

+ (instancetype)videoPlayerWithFrame:(CGRect)frame andDelegate:(id <WLVideoPlayerDelegate>) mdelegate;
...
// 展示在那个view上 占位图
- (void)showInView:(UIView *)view andThumb:(UIImage *)thumb;
// 能否旋转
